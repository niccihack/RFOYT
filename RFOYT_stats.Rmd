...
#Export certain columns for comparison to mRNA
justigf <- select(growth, PITtag, IGF1)
write.csv(justigf,'igf1.csv')
...
igflm = lm(IGF1~treatment, data = igf1.num, na.action=na.omit)
igfanova = anova(igflm)
knitr::kable(igfanova, caption = 'Plasma IGF-1 ANOVA')
igf1 = dplyr::filter(igf1.num,!is.na(igf1.num$IGF1)) %>% group_by(treatment)%>% dplyr::summarise(average_igf = mean(IGF1), sem_igf =  sd(IGF1)/sqrt(length(IGF1)),n=length(IGF1))
igf1$text = c('**','')
ggplot(igf1, aes(x = treatment, y = average_igf))+
  ...
  scale_fill_manual(values = c("#696969","#A9A9A9"))+  
  coord_cartesian(ylim = c(15,25))+
  scale_x_discrete(labels = c("High","Low"))+
  geom_errorbar(aes(ymin=average_igf - sem_igf, ymax=average_igf + sem_igf), width = 0.25, size = 1)+
  geom_signif(aes(y = average_igf), y_position=24, xmin=1, xmax=2,annotation="p = 0.0034", tip_length=0.1, textsize = 7, size = 1, vjust = -0.5) +
  theme_classic(base_size = 20)+
  theme(legend.position = 'none',axis.ticks.x=element_blank(),axis.text.x = element_text(size = 20), axis.text.y = element_text(size = 20))
...
mean.mass = group_by(b.mass,day,treatment) %>% dplyr::summarise(mean_mass=mean(value),sem_mass = sd(value)/sqrt(length(value)),n=length(value))
mean.mass$sig = '*'
...
ggplot(mean.mass, aes(x = day, y =mean_mass, color = treatment))+
  ...
  ylim(c(2.5,13))+
  xlab('')+
  geom_text(aes(label = sig),nudge_y=0.75, show.legend = FALSE, size= 15, color = 'black')+
  scale_color_manual(values = c("#696969","#A9A9A9"), name = 'Treatment', labels = c('High ration','Low ration'))+
  ...
  theme(legend.position = c(0.22, 0.79), axis.text.y = element_text(size = 22),axis.text.x = element_text(size = 22), legend.text = element_text(size = 26), legend.title = element_text(size = 28))
ggsave(filename = 'Figure1a.tiff', width = 10, height = 5)
```
...
ggplot(mass.day0.98, aes(x = IGF1, y = sgr.mass)) + 
  ...
  ylab('SGR Mass (% / day)')+
  theme_classic(base_size = 20)+
  theme(legend.background = element_rect(linetype = 'solid', size = 0.5, colour = 'black'), axis.text.x = element_text(size = 20), axis.text.y = element_text(size = 20),legend.position = c(0.87,0.15),plot.margin=unit(c(1,1,1.5,1.2),"cm"))
ggsave(filename = 'Fig4a.tiff', width = 7, height = 5)
```
```{r, include=F}
anova(lm(sgr.mass~treatment,data = dplyr::filter(dat.mass, Comb == 'Day-0-98')))
#For manuscript
last.sgr.mass <- dplyr::filter(dat.mass, Comb == 'Day-0-98')%>% group_by(treatment) %>% dplyr::summarize(average_sgr = mean(sgr.mass), sem_sgr =  sd(sgr.mass)/sqrt(length(sgr.mass)),n=length(sgr.mass))
ggplot(last.sgr.mass, aes(x = treatment, y = average_sgr, fill = treatment))+
  ...
  scale_x_discrete(labels = c("High","Low"))+
  geom_errorbar(aes(ymin=average_sgr - sem_sgr, ymax=average_sgr + sem_sgr), width = 0.25, size = 1)+
  theme_classic(base_size = 20)+
  geom_signif(aes(y = average_sgr), y_position=2.7, xmin=1, xmax=2,annotation="p < 0.0001", vjust = -0.5, tip_length=0.2, textsize = 6) +
  theme(legend.position = 'none',axis.ticks.x=element_blank(),axis.text.x = element_text(size = 20), axis.text.y = element_text(size = 20), plot.margin=unit(c(1,1,1.5,1.2),"cm"))
...
ggplot(mean.length, aes(x = day, y = mean_length, color = treatment))+
  ...
  ylim(c(55,90))+
  geom_point(size = 4)+
  geom_text(aes(label = sig),nudge_y=3,size = 15, color = 'black', show.legend = FALSE)+
  geom_errorbar( aes(ymin=mean_length - sem_length, ymax = mean_length + sem_length), width = 1.5)+
  scale_color_manual(values = c("#696969","#A9A9A9"), name = 'Treatment', labels = c('High ration','Low ration'))+  
  scale_x_continuous(expand = c(0,0),limits = c(-10,108),breaks = c(0,24,48,75,91,98))+ 
  theme_classic(base_size = 24)+
  theme(legend.position = 'none',axis.text.y = element_text(size = 22,margin = margin(l=10)),axis.text.x = element_text(size = 22))
ggsave(filename = 'Figure1b.tiff', width = 10, height = 5)
```
```{r, include=F}
#presentation graph
ggplot(mean.length, aes(x = day, y = mean_length, color = treatment))+
...
last.sgr.sl <- sl.day0.98 %>% group_by(treatment) %>% dplyr::summarize(average_sgr = mean(sgr.sl), sem_sgr =  sd(sgr.sl)/sqrt(length(sgr.sl)),n=length(sgr.sl))
last.sgr.sl$text = c('***','')
ggplot(last.sgr.sl, aes(x = treatment, y = average_sgr, fill = treatment))+
  ylab('SGR SL (% / day)')+
  xlab('Ration Treatment')+
  geom_bar(stat = 'identity', color = 'black', aes(fill = treatment),position=position_dodge(.9))+
  scale_fill_manual(values = c("#696969","#A9A9A9"))+  
  ...
  geom_errorbar(aes(ymin=average_sgr - sem_sgr, ymax=average_sgr + sem_sgr), width = 0.25, size = 1)+
  theme_classic(base_size = 20)+
  geom_signif(aes(y = average_sgr), y_position=3.7, xmin=1, xmax=2,annotation="p < 0.0001", vjust = -0.5, tip_length=0.2, textsize = 6) +
  theme(legend.position = 'none',axis.ticks.x=element_blank(),axis.text.x = element_text(size = 20), axis.text.y = element_text(size = 20), plot.margin=unit(c(1,1,1.5,1.2),"cm"))
...
ggplot(sl.day0.98, aes(x = IGF1, y = sgr.sl)) + 
  ...
  ylab('SGR SL (% / day)')+
  theme_classic(base_size = 20)+
  theme(legend.position = 'none', axis.text.x = element_text(size = 20), axis.text.y = element_text(size = 20),plot.margin=unit(c(1,1,1.5,1.2),"cm"))
...
ggplot(mean.cf, aes(x = day, y = average_cf, color = treatment))+
  geom_line(size = 2)+
  ylab(expression(paste('Condition Factor ( g / ',cm^3,' )')))+
  ylim(c(1.39,2.2))+
  xlab('Day')+
  geom_point(size = 4)+
  geom_text(aes(label = sig),nudge_y=.07, show.legend = FALSE, size = 15, color = 'black')+
  scale_color_manual(values = c("#696969","#A9A9A9"), name = 'Treatment', labels = c('High ration','Low ration'))+ 
  geom_errorbar(aes(ymin=average_cf - sem_cf, ymax = average_cf + sem_cf), width = 1.5) +
  scale_x_continuous(expand = c(0,0),limits = c(-10,108),breaks = c(0,24,48,75,91,98))+ 
  theme_classic(base_size = 24)+
  theme(legend.position = 'none',axis.text.x = element_text(size = 22),axis.text.y = element_text(size = 22))
ggsave(filename = 'Figure1c.tiff', width = 10, height = 5)
```
