...
ggplot(mean.mass, aes(x = day, y =mean_mass, color = treatment))+
  ...
  ylim(c(2.5,13))+
  xlab('')+
  geom_text(aes(label = sig),nudge_y=0.75, show.legend = FALSE, size= 8, color = 'black')+
...
ggplot(last.sgr.sl, aes(x = treatment, y = average_sgr, fill = treatment))+
  ...
  geom_errorbar(aes(ymin=average_sgr - sem_sgr, ymax=average_sgr + sem_sgr), width = 0.25, size = 1)+
  theme_classic(base_size = 18)+
  geom_text(aes(label = text),nudge_y=0.15, size = 8)+
  theme(legend.position = 'none',axis.ticks.x=element_blank(),axis.text.x = element_text(size = 18), axis.text.y = element_text(size = 18), plot.margin=unit(c(1,1,1.5,1.2),"cm"))
...
ggplot(mean.cf, aes(x = day, y = average_cf, color = treatment))+
  ...
  xlab('Day')+
  geom_point(size = 2)+
  geom_text(aes(label = sig),nudge_y=.05, show.legend = FALSE, size = 8, color = 'black')+
  scale_color_manual(values = c("#696969","#A9A9A9"), name = 'Treatment', labels = c('High ration','Low ration'))+ 
