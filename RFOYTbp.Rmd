...
bp2a, bp2b, igf1, and igf2 have non-normal residuals.
...
bp2b definitely has a positive skew. igf1 & 2 & bp2a have a slight positive skew but look normal overall.
...
ggplot(igfs, aes(x = gene, weight = new_mean, ymin=new_mean - new_sem, ymax=new_mean + new_sem, fill = forcats::fct_rev(treatment)))+
  ...
  geom_signif(aes(y = new_mean), y_position=2.4, xmin=0.7, xmax=1.3,annotation="***", tip_length=0, textsize = 6) +
  theme_classic(base_size = 18)+
  theme(axis.text.x = element_text(face = "italic"),axis.title.x = element_blank(),plot.margin=unit(c(1,1,1.5,1.2),"cm"), legend.position = c(0.8,0.85))
...
ggplot(bps, aes(x = gene, weight = new_mean, ymin=new_mean - new_sem, ymax=new_mean + new_sem,  fill = forcats::fct_rev(treatment)))+
  ...
  scale_x_discrete(labels = c("bp2a","bp2b",'bp5a','bp5b'))+
  scale_fill_manual(values = c("#A9A9A9","#696969"), name = 'Treatment', labels = c('High','Low'))+
  geom_signif(aes(y = new_mean), y_position=c(1.5,1.5), xmin=c(2.7, 3.7), xmax=c(3.2, 4.2),annotation=c("*", "**"), tip_length=0, textsize = 6) +
  theme_classic(base_size = 18)+
  theme(axis.text.x = element_text(face = "italic"), legend.position = 'none',axis.title.x = element_blank(),plot.margin=unit(c(1,1,1.5,1.2),"cm"))
...
ggplot(receps, aes(x = gene, y = new_mean, ymin=new_mean - new_sem, ymax=new_mean + new_sem, fill = forcats::fct_rev(treatment)))+
  ...
  theme_classic(base_size = 18)+
  theme(axis.text.x = element_text(face = "italic"),axis.title.x = element_blank(),legend.position = 'none',plot.margin=unit(c(1,1,1.5,1.2),"cm"))
ggsave(filename = 'Fig7c.tiff', width = 7, height = 5)

```

Plasma and mRNA igf1 correlation

```{r}
pigf <- read_csv("igf1.csv")[,2:3]#pulled plasma igf1 from growth data
migf <- filter(out_Cq, gene == 'migf1')#only want to look at correlation to liver igf1
...
ggplot(cmigf1, aes(x = IGF1, y = Cq_normalized)) + 
  ...
  theme_classic(base_size = 18)+
  theme(legend.background = element_rect(linetype = 'solid', size = 0.5, colour = 'black'), legend.position = c(0.2,0.8))
...
ggplot(cmigf1.h, aes(x = IGF1, y = Cq_normalized)) + 
  ...
  ylab(expression(paste('Relative liver ',italic('igf1'), ' mRNA')))+
  theme_classic(base_size = 18)+
  theme(legend.background = element_rect(linetype = 'solid', size = 0.5, colour = 'black'), legend.position = 'none')
...
ggplot(cmigf1.l, aes(x = IGF1, y = Cq_normalized)) + 
  ...
  theme_classic(base_size = 18)+
  theme(legend.background = element_rect(linetype = 'solid', size = 0.5, colour = 'black'), legend.position = 'none')
ggsave(filename = 'FigS2c.tiff', width = 7, height = 5)
```
...
ggplot(igfs.l, aes(x = gene, weight = new_mean, ymin=new_mean - new_sem, ymax=new_mean + new_sem, fill = forcats::fct_rev(treatment)))+
  ...
  scale_fill_manual(values = c("#A9A9A9","#696969"), name = 'Treatment', labels = c('Low ration','High ration'))+
  theme_classic(base_size = 18)+
  theme(axis.text.x = element_text(face = "italic"),axis.title.x = element_blank(),plot.margin=unit(c(1,1,1.5,1.2),"cm"))
...
ggplot(bps.liv, aes(x = gene, weight = new_mean, ymin=new_mean - new_sem, ymax=new_mean + new_sem, fill = forcats::fct_rev(treatment)))+
  ...
  geom_signif(aes(y = new_mean), y_position=c(1.4,1.3), xmin=c(0.7,1.7), xmax=c(1.3,2.3),annotation=c("**","***"), tip_length=0, textsize = 6) +
  theme_classic(base_size = 18)+
  theme(axis.text.x = element_text(face = "italic"),axis.title.x = element_blank(),legend.position = 'none',plot.margin=unit(c(1,1,1.5,1.2),"cm"))
